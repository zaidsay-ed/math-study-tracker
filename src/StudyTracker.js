    ...
    <div className="p-4 max-w-5xl mx-auto bg-gray-900 min-h-screen text-white">
      ...
      <div className="space-y-4">
        {studyPlan.map((item, index) => (
          <div
            key={index}
            className={`flex items-center justify-between p-4 border rounded-2xl shadow-md transition-all duration-200 ${checked[index] ? "bg-green-800 border-green-500" : "bg-gray-800 border-gray-700"}`}
          >
            <div>
              <p className="text-sm font-semibold text-gray-300">{item.date} – {item.subject}</p>
              <p className="text-base font-medium text-white mt-1">{item.topic}</p>
              <p className="text-xs text-gray-400 mt-1 italic">{item.tier}</p>
            </div>
            <input
              type="checkbox"
